	...
	for(p=0; p<n; p++)
	{
		do
		{
			...
			for(q=p+1; q<n; q++)
			{
				...
				aqq = gsl_vector_get(eig,q);
				apq = gsl_matrix_get(A,p,q);
				phi = atan2(SORT*2*apq,SORT*(aqq-app))/2;
				s = sin(phi);
				c = cos(phi);
				app1 = c*c*app-2*s*c*apq+s*s*aqq;
				aqq1 = s*s*app+2*s*c*apq+c*c*aqq;
...
{
	assert(A->size1 == A->size2 && V->size1 == V->size2 && A->size1 == V->size1 && eig->size == V->size1);
	int flag, rot, p, q, i, n = A->size1, maxq, minq;
	double app, aqq, apq, phi, s, c, app1, aqq1;
	double aip, aiq, api, aqi, vip, viq, max, min;
	for(i=0; i<n; i++)
	{
		gsl_vector_set(eig,i,gsl_matrix_get(A,i,i));
	}
	...
	for(p=0; p<n-1; p++)
	{
		...
		{
			flag = 0;
			a = gsl_matrix_subrow(A,p,p+1,n-1-p);
			maxq = gsl_vector_max_index(&a.vector);
			minq = gsl_vector_min_index(&a.vector);
			max = gsl_vector_get(&a.vector,maxq);
			min = gsl_vector_get(&a.vector,minq);
			q = ( fabs(max) >= fabs(min) ) ? p+1+maxq : p+1+minq;
			app = gsl_vector_get(eig,p);
			aqq = gsl_vector_get(eig,q);
			apq = gsl_matrix_get(A,p,q);
			phi = atan2(SORT*2*apq,SORT*(aqq-app))/2;
