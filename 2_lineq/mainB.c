...
int main(void)
{
	...
	double dgsl = gsl_linalg_LU_det(B,i);
	 ...
	 * is evaluated
	 */
	gsl_blas_dgemm(CblasNoTrans,CblasNoTrans,1,&a.matrix,BI,0,B);
	printf("\nCompute the inverse matrix Binv of B. The entrywise norm of B*Binv - I is\n");
	printf("\t|B*Binv - I| =\t");
	d = 0;
	for(i=0; i < COL; i++)
	{
		gsl_matrix_set(B,i,i,gsl_matrix_get(B,i,i)-1);
		for(j=0; j < COL; j++)
		{
			d += pow(gsl_matrix_get(B,i,j),2);
		}
	}
	printf("%g\n",sqrt(d));
