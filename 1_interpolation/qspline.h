...
qspline* qspline_alloc(int n, double x[], double y[])
{
	...
	for(i = 1; i<n-1; i++)
	{
		s->c[i] = (p[i]-p[i-1]-s->c[i-1]*s->h[i-1])/s->h[i];
	}
	...
	for(i=n-2; i>0; i--)
	{
		s->c[i-1] = (p[i]-p[i-1]-s->c[i]*s->h[i])/s->h[i-1];
	}

	// estimating b[i]
	for(i=0; i<n-1; i++)
	{
		s->b[i] = p[i] - s->c[i]*s->h[i];
	}
...
double qspline_integral(qspline *s, double z)
{
	...
	for(j=0; j<i; j++)
	{
		zint += s->y[j]*s->h[j] + s->b[j]*pow(s->h[j],2)/2.0 + s->c[j]*pow(s->h[j],3)/3.0;
	}
	
	// the integral for x[i] to z
	h = z - s->x[i];
	zint += s->y[i]*h + s->b[i]*pow(h,2)/2.0 + s->c[i]*pow(h,3)/3.0;
